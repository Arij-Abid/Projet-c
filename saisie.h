...
//int test=0;
    ...
    if(strlen(textsaved)==0 || valide_nom(textsaved)==0)
    {
        MessageBox(hwnd,"Champ vide ou il existe des valeurs numérique","Erreur nom",MB_OK);
    ...
    else
    {
        ...
        puts(reper.nom);
        fflush(stdin);
        GetWindowText(prenom,textsaved,20);
    if(strlen(textsaved)==0 || valide_nom(textsaved)==0)
    {
        MessageBox(hwnd,"Champ vide ou il existe des valeurs numérique","Erreur Prenom",MB_OK);
    }
    else
    {
        fflush(stdin);
        ...
        if(valide_date()==0)
        {
                MessageBox(hwnd,"Date non valide ou champ vide","Erreur date",MB_OK);

        }
        else
        {
            fflush(stdin);
                GetWindowText(jour,reper.dn.j,20);
                fflush(stdin);
                GetWindowText(mois,reper.dn.m,20);
                fflush(stdin);
                GetWindowText(annee,reper.dn.a,20);
                puts(reper.dn.j);
                puts(reper.dn.m);
                puts(reper.dn.a);
    ...
    if(strlen(textsaved)==0 || valide_nom(textsaved)==0)
    {
        MessageBox(hwnd,"Champ vide ou il existe des valeurs numérique","Erreur Ville",MB_OK);
    }
    else
    {
        fflush(stdin);
...
liste ajouter_position (liste l)
{
repertoire reper;
    ...
    else
    {
       ...
       else
       {
           ...
           GetWindowText(position,textsaved,20); // c: text fiel ; rep.numtel : placement d'enregistrement ; 20 : 20-1=19 le nombre des caractéres lire
                      ...
                      if(atoi(textsaved)>=taille+1 || atoi(textsaved)==0  || strlen(textsaved)==0 || atoi(textsaved)==1 )
                      {
...
/******************Fonction Suppression tout num******************/
liste suppression_all(liste l)
{
    liste p1=l;
...
liste suppression_ville(liste l,char ville[20] )
{
    liste p=l, prec=NULL;
    while(p)
    {
        if(strcmp(p->info.adresse.ville,ville)==0)
        {
            if(prec==NULL)
            {
                l=l->suiv;
...
liste recuperation_repertoire()
{
    ...
    if(fp==NULL)
    {
       ...
       while(fgets(ligne,100,fp)!=NULL)
       {
            ...
            pos=construction(ligne,pos,'/',e.dn.j);
            pos=construction(ligne,pos,'/',e.dn.m);
            pos=construction(ligne,pos,'|',e.dn.a);
...
void enre_tt(liste l)
{
    ...
    {
        i++;
        fprintf(fp," %d|\t%s\t%s\t\t%s\t\t%s/%s/%s\t%s\t%s\t\t%s\t\t%s\n",i,l->info.numtel,l->info.nom,l->info.prenom,l->info.dn.j,l->info.dn.m,l->info.dn.a,l->info.adresse.rue,l->info.adresse.ville,l->info.adresse.codep,l->info.email);
        fprintf(fp,"-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n");
        l=l->suiv;
